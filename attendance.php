		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					...
					//
					$sql = "INSERT INTO attendance (employee_id, date, time_in,time_out,num_hr, status) VALUES ('$id', '$date_now', NOW(),'00:00:00',0, '$logstatus')";
					if($conn->query($sql)){
					...
					if($row['time_out'] != '00:00:00'){
						...
						if($conn->query($sql)){
							...
							if(isset($_POST['break'])){
								...
								$int  = round(abs($time_out - $time_in) / 3600,2)-0.5;
								$output['error'] = true;
								$output["message"] =  'Time out: '.$row['firstname'].' '.$row['lastname'];
								//echo json_encode($output);
							}
							else{
								$int = $difference = round(abs($time_out - $time_in) / 3600,2);
								$output['error'] = true;
								$output["message"] =  'Time out: '.$row['firstname'].' '.$row['lastname'];
								//echo json_encode($output);
							}
